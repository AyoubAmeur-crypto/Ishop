...
import { getCatUserDetails } from '@/app/api/product/cart'
import { placeOrder } from '@/app/api/product/order'
import { createOrderFromCart } from '@/app/api/product/order'
...
function CartCheckout() {
    ...
    const getCartDetails = async () => {
        ...
        try {
            ...
            console.log("cart server response ", cartRes);
    ...
    const calculateSavings = () => {
        return cartItems.reduce((total, item) => {
            const price = item.product?.price || 0
            return total + (price * 0.2 * item.quantity) // 20% savings
        }, 0)
    }
    ...
    const calculateOriginalPrice = () => {
        return cartItems.reduce((total, item) => {
            const price = item.product?.price || 0
            return total + (price * 1.2 * item.quantity) // Original price with 20% markup
        }, 0)
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const serverRes = await createOrderFromCart(cartOrderData)
            ...
            <ProtectedRoutes>
                <div className="min-h-screen bg-black flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-white text-xl">Cart is empty</p>
                        <button
                            onClick={() => window.location.href = '/en/cart'}
                            className="mt-4 bg-gradient-to-r from-yellow-400 to-yellow-500 text-black px-6 py-3 rounded-xl font-bold"
                        >
                            Return to Cart
                        </button>
                    </div>
    ...
    const savings = calculateSavings().toFixed(2)
    const originalPrice = calculateOriginalPrice().toFixed(2)
                        ...
                        <div className="flex-1 max-w-2xl">
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-yellow-400 text-sm font-medium">Cart Checkout</p>
                                </div>
                            </div>
                ...
                <div className="max-w-7xl mx-auto px-4 py-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2">
                            ...
                            {/* Step 2: Enhanced Delivery Information */}
                            {currentStep === 2 && (
                                <div className="bg-gradient-to-br from-black/60 to-black/40 border border-yellow-400/30 rounded-3xl p-4 sm:p-8 backdrop-blur-sm">
                                    <div className="flex items-center gap-4 mb-6 sm:mb-8">
                                        <div className="bg-gradient-to-br from-yellow-400 to-yellow-500 p-3 rounded-2xl">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-400 text-sm sm:text-base">Where should we deliver your {cartItems.length} item{cartItems.length > 1 ? 's' : ''}?</p>
                                        </div>
                                    </div>
                                ...
                                <div className="bg-gradient-to-br from-black/60 to-black/40 border border-yellow-400/30 rounded-3xl p-4 sm:p-8 backdrop-blur-sm">
                                    <div className="flex items-center gap-4 mb-8">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-400">Review your cart items before completing purchase</p>
                                        </div>
                                    </div>
                                    
                                    {/* Enhanced Cart Items Summary */}
                                    <div className="bg-gradient-to-br from-black/40 to-black/20 border border-yellow-400/20 rounded-2xl p-6 mb-6">
                                        <h3 className="text-xl font-bold text-white mb-6">Cart Summary ({cartItems.length} items)</h3>
                                        
                                        {/* Cart Items List */}
<div className="space-y-4 mb-6">
                                            {cartItems.map((item) => (
                                                <div key={item._id} className="flex items-center gap-4 p-4 bg-black/30 rounded-xl border border-yellow-400/10">
                                                    <div className="relative">
                                                        <img 
                                                    ...
                                                    <div className="flex-1">
                                                        <h4 className="text-white font-semibold text-sm mb-1">{item.product?.title}</h4>
                                                        <p className="text-gray-400 text-xs mb-2">{item.product?.brand} • {item.product?.category}</p>
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <div className="flex items-center">
                                                                    {[...Array(5)].map((_, i) => (
                                                                        <Star 
                                                                            key={i} 
                                                                            className={`w-3 h-3 ${i < Math.floor(item.product?.ratings || 0) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'}`}
                                                                        />
                                                                    ))}
                                                                </div>
                                                                <span className="text-yellow-400 text-xs">{item.product?.ratings}</span>
                                                            </div>
                                   ...
                                   <div className="bg-gradient-to-br from-black/40 to-black/20 border border-yellow-400/20 rounded-2xl p-3 mb-6">
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0 mt-6 sm:mt-8">
                                        ...
                                        <button
                                            ...
                                            className="order-1 sm:order-2 bg-gradient-to-r from-green-500 to-green-600 text-white px-8 sm:px-12 py-3 sm:py-4 rounded-xl font-bold text-base sm:text-lg hover:from-green-600 hover:to-green-700 transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 shadow-lg hover:shadow-xl hover:scale-105"
                                        >
                                ...
                                <div className="bg-gradient-to-br from-black/60 to-black/40 border border-yellow-400/30 rounded-3xl p-4 sm:p-8 backdrop-blur-sm">
                                    ...
                                    <div className="text-center mb-8">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
                                                ...
                                                Your {cartItems.length} item{cartItems.length > 1 ? 's are' : ' is'} on the way to you.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-400/30 rounded-2xl p-6 mb-8">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            {/* Products Info */}
                                            <div className="space-y-4">
                                                <h4 className="text-white font-semibold">Items Ordered:</h4>
                                                <div className="space-y-2 ">
                                                    {cartItems.slice(0, 3).map((item) => (
                                                        <div key={item._id} className="flex items-center gap-3">
                                                            <img 
                                                                src={item.product?.image} 
                                                                alt={item.product?.title}
                                                                className="w-10 h-10 object-cover rounded border border-green-400/30"
                                                            />
                                                            <div className="flex-1">
                                                ...
                                                <p className="text-green-400 font-bold">Total: ${totalPrice}</p>
                                            </div>
                                    ...
                                    <div className="bg-gradient-to-br from-black/80 to-black/60 border border-yellow-400/30 rounded-3xl p-8 max-w-md w-full mx-4 text-center">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-gray-300 text-sm">
                                                Please don't close this window. Your order with {cartItems.length} item{cartItems.length > 1 ? 's' : ''} is being processed securely.
                                            </p>
                                        </div>
                                        ...
                                        <div className="mt-8 space-y-3">
                                            <div className="flex items-center gap-3 text-sm">
                                                ...
                                                <span className="text-green-400">Validating cart items</span>
                                            </div>
                                            <div className="flex items-center gap-3 text-sm">
                        ...
                        <div className="order-first lg:order-last lg:sticky lg:top-4 lg:h-fit space-y-4 sm:space-y-6">
                            ...
                            <div className="bg-gradient-to-br from-black/60 to-black/40 border border-yellow-400/30 rounded-3xl p-6 backdrop-blur-sm">
                                ...
                                {/* Cart Items Preview */}
                                <div className="bg-gradient-to-br from-black/40 to-black/20 rounded-2xl p-4 mb-6 border border-yellow-400/20">
                                    <p className="text-yellow-400 font-semibold mb-3">{cartItems.length} Items in Cart</p>
                                    <div className="space-y-3 ">
                                        {cartItems.map((item) => (
                                            <div key={item._id} className="flex items-center gap-3">
                                                <img 
                            ...
                            <div className="bg-gradient-to-br from-black/40 to-black/20 border border-yellow-400/20 rounded-2xl p-4 sm:p-6">
